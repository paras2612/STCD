...
def findcauses(target, cuda, epochs, kernel_size, layers, 
               log_interval, lr, optimizername, seed, dilation_c, significance, file,distances):
    ...
    #print(scores, torch.max(scores), torch.min(scores), torch.mean(scores))
    #print(len(scores[scores > 0]))
    dist = distances[targetidx,indices]
    cols_i = pd.read_csv(file).columns
    pot_idx = []
    c=0
    for i in range(len(dist)):
        if (float(cols_i[i].split("  ")[1]) >= float(target.split("  ")[1])):
            scores[i] += 0.001 * (dist[i])
            pot_idx.append(i)
            c+=1
        else:
            scores[i] = scores[i]*-1
    print("NUMBER OF NODES SPATIALLY LOCATED ABOVE ",target.split("\n")[0]," IS : ",c)
    #print(len(scores[scores > 0]))
    #print("AFTER PENALIZING DISTANCE")
    #print(scores, torch.max(scores), torch.min(scores), torch.mean(scores))
    potentials = []
    #attention interpretation to find tau: the threshold that distinguishes potential causes from non-causal time series
    #print(torch.max(scores),torch.min(scores),torch.mean(scores))
    # print(torch.mean(scores))
    # #for i in indices:
    # print(len(indices))
    # print(len(pot_idx))
    for i in indices:
        #if scores[i] > torch.mean(scores):
        if scores[i] > torch.mean(scores) and scores[i] > 0.0:
            potentials.append(i)
    '''if len(s)<=5:
        potentials = []
        for i in indices:
    ...
    # print("Potential causes: ", sorted(potentials,reverse=False))
    ...
    for idx in potentials:
        ...
        #print("THIS IS DIFF")
        #print("THIS IS DIFF*significane",round(diff*significance,4))
        #print("THIS IS TEST_DIFF",round(testdiff,4))
        testdiff = round(testdiff,4)
        diff = round(diff,4)
    ...
    print("Scores: ",scores[validated])
    np.savetxt("/home/local/ASUAD/psheth5/Downloads/TCDF-master/validatedScores.txt",scores[validated].cpu().detach().numpy())
    np.savetxt("/home/local/ASUAD/psheth5/Downloads/TCDF-master/validatedIndex.txt",validated)
    return
    idx=[]
    for i in range(len(scores[validated])):
        if(scores[validated][i]>torch.mean(scores[validated])):
            idx.append(i)
